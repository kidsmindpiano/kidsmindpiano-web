...
import { Music, Palette, Heart, Gamepad2, Trophy, Users } from "lucide-react";
...
const curriculum = [
  { period: "3개월", items: ["기본 자세와 손 모양", "계이름과 리듬 읽기", "동요 1곡 완주 🎉"], color: "bg-accent/10 text-accent" },
  { period: "6개월", items: ["연주회 목표 설정", "양손 연주 시작", "월별 학부모 상담"], color: "bg-primary/10 text-primary" },
  { period: "1년+", items: ["K-Pop / 영화 OST 도전", "즉흥 연주 심화", "연주회 MC & 리더십"], color: "bg-secondary/10 text-secondary" },
];
    ...
    <div>
      ...
      <section className="bg-gradient-to-br from-primary/5 to-accent/5 py-16 md:py-24">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <motion.p className="text-lg text-muted-foreground" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
            음악치료 전문가가 설계한 커리큘럼, Zoom 1:1 맞춤 레슨
          </motion.p>
        </div>
      </section>
      ...
      <section className="py-16 max-w-6xl mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {[
            { icon: Users, title: "Zoom 1:1 수업", desc: "선생님과 아이, 단둘이 집중! 외장 카메라와 마이크로 선명한 화면과 소리를 전달합니다." },
            { icon: Palette, title: "디지털 악보 활용", desc: "색깔 펜으로 시각화 — A부분은 노랑, B부분은 파랑. 악보가 그림처럼 보여요." },
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {roulette.map((r, i) => (
              <motion.div key={r.name} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={i}>
                <Card className="text-center border-0 shadow-sm hover:shadow-md transition">
            ...
            <motion.div key={c.period} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={i}>
              <Card className="h-full border-0 shadow-md">
                <CardContent className="p-8">
                  <span className={`inline-block px-3 py-1 rounded-full text-sm font-bold mb-4 ${c.color}`}>{c.period}</span>
